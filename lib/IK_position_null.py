...
class IK:
    ...
    def displacement_and_axis(target, current):
        ...
        rot_cur = current[:-1,:-1] 
        rot_target = target[:-1,:-1] 

        axis = calcAngDiff(rot_target, rot_cur) 
    ...
    def distance_and_angle(G, H):
        ...
        distance = G[:3, -1] - H[:3, -1]
        distance = np.linalg.norm(distance)

        rot_G = G[:3, :3]
        rot_H = H[:3, :3]
        rot_diff = np.dot(rot_G.T, rot_H)

        trace = 0.5 * (np.trace(rot_diff) - 1)
    ...
    def is_valid_solution(self,q,target):
        ...
        ## STUDENT CODE STARTS HERE
        fk = FK()
        pos_cur = fk.forward(q)[1]
        distance, angle = IK.distance_and_angle(target, pos_cur)
        success = True

        if np.any(q<self.lower) or np.any(q > self.upper): 
            # success = False
            success = True
            message = "Joint exceeds its limit"
            print(message)
        elif (angle > self.angular_tol) or (distance > self.linear_tol): 
            # success = False
            success = True
            message = "Angle exceeds tolerance"
            print(message)
        else:
            success = True
    ...
    def end_effector_task(q,target,method):
        ...
        fk = FK()
        J = calcJacobian(q)
        current_position = fk.forward(q)[1]

        displacement, axis = IK.displacement_and_axis(target, current_position)
        ee_vel = np.hstack((displacement, axis.flatten()))

        if method == "J_pseudo": 
            dq = np.linalg.pinv(J) @ ee_vel
        else:
            dq = J.T @ ee_vel
    ...
    def inverse(self, target, seed, method, alpha):
        ...
        while True:
            ...
            dq_primary = IK.end_effector_task(q,target, method)

            # Secondary Task - Center Joints
            dq_secondary = IK.joint_centering_task(q)
            
            # Task Prioritization
            J = calcJacobian(q)
            J_pseudo = np.linalg.pinv(J)

            dq = alpha * (dq_primary + (np.eye(7) - J_pseudo @ J) @ dq_secondary)

            # Check termination conditions
            if np.linalg.norm(dq) < self.min_step_size or i >= self.max_steps:
                break
